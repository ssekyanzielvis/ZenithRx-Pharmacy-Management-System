...
CREATE TABLE IF NOT EXISTS patient_profiles (
  id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  auth_user_id        UUID UNIQUE, -- Optional link to auth.users if Supabase Auth is enabled
  full_name           TEXT NOT NULL,
  phone               TEXT NOT NULL UNIQUE,
  email               TEXT,
  pin_hash            TEXT, -- For lightweight PIN-based Mobile PWA logins
  age                 INTEGER CHECK (age > 0 AND age < 150),
  gender              TEXT CHECK (gender IN ('Male', 'Female', 'Other')),
  blood_group         TEXT,
...
CREATE TABLE IF NOT EXISTS health_education_articles (
  id             UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  title          TEXT NOT NULL,
  category       TEXT NOT NULL CHECK (category IN ('Hypertension', 'Diabetes', 'Antibiotic Stewardship', 'Malaria', 'Maternal Health', 'General Wellness')),
...
-- ─── 9. ROW LEVEL SECURITY (RLS) FOR PATIENT SUBSYSTEM ────────────────────────
...
-- Health education is readable by all authenticated and anonymous clients
CREATE POLICY "articles_read_public" ON health_education_articles
  FOR SELECT USING (is_published = TRUE);

-- Patients can access only their own records, and Super Admin / Supervising Pharmacist can view for clinical fulfillment
CREATE POLICY "patient_profiles_access" ON patient_profiles
  FOR ALL USING (auth_user_id = auth.uid() OR is_super_admin() OR auth.uid() IS NULL);
